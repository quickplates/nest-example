...
COPY package.json pnpm-lock.yaml .npmrc Taskfile.dist.yaml ./

# Install root dependencies
RUN task install -- --frozen-lockfile
...
COPY --from=build /build/.pnpm-store/ ./.pnpm-store/
